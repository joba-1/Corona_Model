...
directory = './input_data/demographics/'

hhds_per_type = pd.read_csv(directory + 'household_type_relative_share.csv', index_col=0)
hhd_size_dist_per_hhd_type = pd.read_csv(
    directory + 'household_size_distributions_per_household_type.csv', index_col=0)
age_dist_per_hhd_type = pd.read_csv(
    directory + 'age_distribution_germany_per_household_type_and_year.csv', index_col=0)
...
for hhd_type in list(hhd_size_dist_per_hhd_type.T):
    ...
    # Get indices of elements that are non-zero
    keeplist = [i for i in range(len(dist)) if dist[i] != 0.0]
...
def initialize_household():
    ...
    if hhd_type == 'Einpersonenhaushalte (Singlehaushalte)':
        ...
        # Increase standard deviation as a function of age1 (NOTE THIS IN WIKI)
