...
hhd_types_probabilites = [p for p in hhds_per_type['%']]
...
def initialize_household():
    ...
    hhd_type = npr.choice(hhd_types_list, 1, hhd_types_probabilites)[0]
    ...
    hhd_mmbrs = npr.choice(hhd_sizes_to_sample, 1, hhd_size_probabilities)[0]

    ## Sample ages according to household type, then return household
    if hhd_type == 'Einpersonenhaushalte (Singlehaushalte)':
    
        age = npr.choice(ages_to_sample, 1, age_dist)[0]
        ...
        age1 = npr.choice(ages_to_sample, 1, age_dist)[0] 
        ...
        ages = list(npr.choice(ages_to_sample[0:18], 1, age_dist[0:18]))
        ## Flip a coin if the next kids are younger or older
        next_kid_younger = npr.choice([-1, 1], 1, [0.5, 0.5])
    
        ## Sample ages for the next kids
        for i in range(1, hhd_mmbrs - 2):
        ...
        ages = list(npr.choice(ages_to_sample, hhd_mmbrs, age_dist)) 
